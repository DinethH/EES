...
if (isset($_POST['photos'])) {
	...
	echo "<br />";
	echo "<div>";
	echo "<table style='width:700px'><tr class='fade-area-1'>";
	$columCount = 0;
	foreach ($file_names = getThumbNames($row_photos['images'], $row_photos['ID']) as $file_name) {
		echo '<td><div align="center" style="max-width:175"><a href="javascript:void(viewer.show('.$columCount.'))"><img style="max-height:160px;max-width:160px;" class="fieldCampImages shadowed"  src="'.$file_name.'"/></a></div></td>';
		$columCount++;
		if (($columCount%4)==0)

  		...
  		<?php
			...
			while ($row = mysql_fetch_array($resultsBusPeople)) {
				...
				echo ' >';
					...
					echo '<td style="padding-left:10px;">'.stripslashes(trim(ucfirst($row['lastName']))).",".' '.stripslashes(trim(ucfirst($row['firstName']))).'</td>';
													...
													ON 
														fieldtriplabs.ID=fieldtrippeople.section 
															WHERE (fieldtripdates.ID=$_POST[date] && 
																fieldtrippeople.status='pending' &&
																fieldtriptransportation.ID = 1) ORDER BY fieldtrippeople.lastName";
			$resultsCU = mysql_query($sqlCU);
			while ($row = mysql_fetch_array($resultsCU)) {
				echo '<tr height="30" ';
				if ($count%2 == 0)
					echo $evenColor;
				echo ' >';
					echo '<td style="padding-left:10px;text-align:right;padding-right:10px; color:red">'.$count.'</td>';
					echo '<td style="padding-left:10px; color:red">'.stripslashes(trim(ucfirst($row['lastName']))).",".' '.stripslashes(trim(ucfirst($row['firstName']))).'</td>';
					echo '<td style="padding-left:10px; color:red">'.$row['section'].'</td>';
				echo '</tr>';
				$count++;
			}
			//unconfirmed bus ends
			if ($count==1) {
				echo '<tr height="30" ';
				if ($count%2 == 0)
					echo $evenColor;
												...
												fieldtriplabs 
													ON 
														fieldtriplabs.ID=fieldtrippeople.section 
															WHERE (fieldtripdates.ID=$_POST[date] && 
																fieldtrippeople.status='pending' &&
																fieldtriptransportation.ID = 2) ORDER BY fieldtrippeople.lastName";
			$resultsCU = mysql_query($sqlCU);
			while ($row = mysql_fetch_array($resultsCU)) {
				echo '<tr height="30" ';
				if ($count%2 == 0)
												...
												fieldtriplabs 
													ON 
														fieldtriplabs.ID=fieldtrippeople.section 
															WHERE (fieldtripdates.ID=$_POST[date] && 
																fieldtrippeople.status='pending'
																) ORDER BY fieldtrippeople.lastName";
...
/*	echo '  <tr height="30">
  ...
  ?>  
        ...
        <table>
        <td headers="30"></td>
        <tr colspan="2"><td style="color:#F00">NOTE: If your name is in red, you have not confirmed your submission. Please do so by following the instructions in the email sent after signing up. If you're having any issues confirming, please contact the webmaster at <a href="mailto:dineth.hettiarachchi@mavs.uta.edu">dineth.hettiarachchi@mavs.uta.edu</a></td></tr>
        </table>
        <?php } ?>
     <?php } ?>
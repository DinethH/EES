 				...
 				<?php
					...
					while($row=mysql_fetch_array($results)) {
						if ($row['ID']==$_GET['user']) {
							...
							if (isset($row['location']) && $row['location']!='NULL' && strlen($row['location'])>0 )
							...
							if (isset($row['email'])&& $row['email']!='NULL' && strlen($row['email'])>0 ) 
								echo "<strong>E-mail</strong>: <a href='mailto:$row[email]'>$row[email]</a><br />" ;
							if (isset($row['website']) && strlen($row['website'])>0 ) 
								echo "<strong>Website</strong>: <a href='$row[website]'>$row[website]</a><br />" ;
							if (isset($row['area']) && $row['area']!='NULL' && strlen($row['area'])>0 ) 

...
<p style="color: blue; font-size: 16px; font-weight: bold">Google Scholar Profile: <a target="_blank" href="http://scholar.google.com/citations?user=CLo_hcUAAAAJ&hl=en&oi=ao">http://scholar.google.com/citations?user=CLo_hcUAAAAJ&hl=en&oi=ao</a></p>
...
<?php
	$sql2 = "SELECT * FROM _basu_publications WHERE year=$year ORDER BY ID DESC";
	...
	while ($row = mysql_fetch_array($results2)) {
		if  ($i%2==0) {
			...
			if (strlen($row['pdf']) > 0) {
				...
				if (strpos($row['pdf'], "http") !== FALSE) {
					echo "<a target='_blank' class='tooltip' href='".$row['pdf']."' >	";
